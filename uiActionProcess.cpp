...
void QT_3DMeasure::actionOpenFile_click()
{
	...
	trans->PostMultiply();
	trans->Scale(1 / VTKWidgtZoomRatio, 1 / VTKWidgtZoomRatio, 1 / VTKWidgtZoomRatio);
...
void QT_3DMeasure::actionCloseCamera_click()
{
	if (CameraThreadTree.size() > 0)
	{
		...
		for (int i = 0; i < Measure3DPointThreadTree.size(); i++)
		{
			Measure3DPointThreadTree[i]->disconnect();
			Measure3DPointThreadTree[i]->release();
			Measure3DPointThreadTree[i]->destroyed();
		}

		std::vector<Measure3DPointThread *>().swap(Measure3DPointThreadTree);
...
void QT_3DMeasure::actionOpenCamera_click()
{
	...
	if (USBCamera_Num > 0 && USBCamera_Num <= MAXCAMERANUM && CameraThreadTree.size() == 0)
	{
		...
		for (int index = 0; index < USBCamera_Num; index++)
		{
			...
			ImgMainProcess * ImgProcessThreadtemp = new ImgMainProcess(CameraQStr.toStdString(), cameraPara.Binarythreshold, cameraPara.CirqueLimit);
