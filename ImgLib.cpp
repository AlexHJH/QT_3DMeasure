...
ImgMainProcess::ImgMainProcess(string str, int threshold_t, double round)
{
	RoundnessLimit = round;
	Segthreshold = threshold_t;
...
void ImgMainProcess::ImgFindContoursProcess()
{
	...
	{
		if (contours[index].size() >(CopyMainImg.cols / 200 * M_PI) && contours[index].size() < (CopyMainImg.cols / 50 * M_PI))
		{
