...
QT_3DMeasure::QT_3DMeasure(QWidget *parent)
	: QMainWindow(parent)
{
	QTextCodec::setCodecForLocale(QTextCodec::codecForName("uf-8"));
	ui.setupUi(this);
	...
	LengthLabel = new QLabel("L: 0", this);
	...
	uiAxes->SetTotalLength(20, 20, 20);
	uiRenderer->AddActor(uiAxes);
	double GirdColor[3] = { 0.5, 0.5, 0.5 };
	uiRenderer->SetBackground(0.3, 0.3, 0.3);
	uiRenderer->AddActor(RenderGird(20, GirdColor, uiRenderWindow));
...
void QT_3DMeasure::CalibrateFromFile()
{
	...
	else
	{
		...
		for (i = 0; i < MatchFileCa.size(); i++)
		{
			...
			WandCalbrateProcess * CameraExternCal = new WandCalbrateProcess((QWorkPath + "/" + ExternFileDirectory + "/" + MatchFileCa[i]).toStdString(), MatchFileCa[i].toStdString());
...
/******************************************************************************************/
	for (int i = 0; i < USBCamera_Num; i++)
	{
		if (i < CameraThreadTree.size())
		{
			...
			QString InterParaDir = DirString + "/InterParament.yml";

			if (di.exists(InterParaDir))
			{
				qDebug() << InterParaDir << "存在" << endl;
			}
			else
			{
				if (QFile::copy(InterParaDirSorce, InterParaDir))
					qDebug() << InterParaDir << "创建成功" << endl;
				else
				{
					qDebug() << "创建失败" << endl;
					return;
			...
			CalibrateThread * CalibrateThreadtemp = new CalibrateThread(DirString.toStdString(), i);
