...
WandCalbrateProcess::WandCalbrateProcess()
{
	...
	WandCheckData.WandDataMatrix.setZero();
	WandCheckData.WandDataMatrix(0, 0) = 0;
	WandCheckData.WandDataMatrix(1, 0) = 150;
	WandCheckData.WandDataMatrix(2, 0) = 600;
	WandCheckData.WandDataMatrix(3, 1) = 300;
	WandCheckData.CheckCount = 200;
}

WandCalbrateProcess::WandCalbrateProcess(WandCheckDatatypedef &cameraPara, string FolderName, string CameraStr)
{
	/******************************************************************/
	////默认初始化数据
	WorkFolder = FolderName;
	WandCheckData = cameraPara;
...
double WandCalbrateProcess::SolveWandCalibrateReProject(Matrix<double, 4, 3> &MarkSiteCamera,
	Matrix<double, 3, 4>& CalibrateMat)
{
	double reProjectErr = 0;
	Matrix<double, 3, 4> TransMattemp = CalibrateMat;
	vector<Point3d> PointWorld;         /////世界坐标系
	vector<Point2d> PointCameraProject; ////重映射相机坐标系
	vector<Point2d> PointCameraOri;     /////原始相机坐标系
	Matrix<double, 3, 3> CameraInterMatrix = WandCheckData.InterParamentMatrix;
	Matrix<double, 4, 3> WandMatrix = WandCheckData.WandDataMatrix;
	...
	for (int j = 0; j < MarkSiteCamera.rows(); j++)
	{
		PointCameraOri.push_back(Point2d(MarkSiteCamera(j, 0), MarkSiteCamera(j, 1)));
	}
	//旋转矩阵转轴角
	cv::Mat rvecMat;
...
double WandCalbrateProcess::SolveWandCalibrateInit(Matrix<double, 4, 3> &MarkSiteCamera, 
...
#define WANDSOLVEFUN 1
	...
	for (i = 0; i < MarkSiteCamera.size(); i++)
	{
		if (i == 0)
		{
			reProErr = SolveWandCalibrateInit(MarkSiteCamera[i], CameraRT, 1);
		}
		else
			reProErr = SolveWandCalibrateReProject(MarkSiteCamera[i], CameraRT);
		//qDebug() << reProErr << endl;
	}
...
void WandCalbrateProcess::RebuildCamera(vector<CameraParamenttypedef> ExternCameraParaMat)
{
...
bool WandCalbrateProcess::WriteWandCalbrateData(string FileName, WandCheckDatatypedef &cameraPara)
{
	FileStorage fout(FileName, FileStorage::WRITE);
	fout.state = FileStorage::UNDEFINED;
	if (fout.isOpened())
	{
		fout.elname = "BinaryTh";
		fout << cameraPara.Binarythreshold;
		fout.elname = "SizeLimit";
		fout << cameraPara.SizeLimit;
		fout.elname = "CirqueLimt";
		fout << cameraPara.CirqueLimit;
		fout.elname = "StraightLimit";
		fout << cameraPara.StraightLimit;
		fout.elname = "MarkNum";
		fout << cameraPara.MarkNum;
		fout.elname = "InterMat";
		fout << cameraPara.InterParamentMat;
		fout.elname = "WandMat";
		fout << cameraPara.WandDataMat;
		fout.elname = "distortMat";
		fout << cameraPara.distortParament;
		fout.elname = "CheckCount";
		fout << cameraPara.CheckCount;
...
bool WandCalbrateProcess::GetWandCalbrateInit(string FileName, WandCheckDatatypedef &cameraPara)
{
	FileStorage fin(FileName, FileStorage::READ);
	if (fin.isOpened())
	{
		...
		fin["BinaryTh"] >> cameraPara.Binarythreshold;
		fin["SizeLimit"] >> cameraPara.SizeLimit;
		fin["CirqueLimt"] >> cameraPara.CirqueLimit;
		fin["StraightLimit"] >> cameraPara.StraightLimit;
		fin["MarkNum"] >> cameraPara.MarkNum;
		fin["InterMat"] >> cameraPara.InterParamentMat;
		cv::cv2eigen(cameraPara.InterParamentMat, cameraPara.InterParamentMatrix);
		fin["WandMat"] >> cameraPara.WandDataMat;
		cv::cv2eigen(cameraPara.WandDataMat, cameraPara.WandDataMatrix);
		fin["distortMat"] >> cameraPara.distortParament;
		fin["CheckCount"] >> cameraPara.CheckCount;
		fin.release();
		return true;
	}
	else
...
bool WandCalbrateProcess::GetWandCalbrateInit(string FileName)
{
	...
	if (fin.isOpened())
	{
		...
		cv::cv2eigen(WandCheckData.InterParamentMat, WandCheckData.InterParamentMatrix);
		fin["WandMat"] >> WandCheckData.WandDataMat;
		cv::cv2eigen(WandCheckData.WandDataMat, WandCheckData.WandDataMatrix);
		fin["distortMat"] >> WandCheckData.distortParament;
...
bool WandCalbrateProcess::WriteWandCalbrateData(string FileName)
{
	...
	{
		fout.elname = "BinaryTh";
		fout << WandCheckData.Binarythreshold;
		fout.elname = "SizeLimit";
		fout << WandCheckData.SizeLimit;
		fout.elname = "CirqueLimt";
		fout << WandCheckData.CirqueLimit;
		fout.elname = "StraightLimit";
		fout << WandCheckData.StraightLimit;
		fout.elname = "MarkNum";
		fout << WandCheckData.MarkNum;
		fout.elname = "InterMat";
		fout << WandCheckData.InterParamentMat;
		fout.elname = "WandMat";
		fout << WandCheckData.WandDataMat;
		fout.elname = "distortMat";
		fout << WandCheckData.distortParament;
		fout.elname = "CheckCount";
		fout << WandCheckData.CheckCount;
...
bool WandCalbrateProcess::WriteFinalCalMat(string FileName, vector<Matrix<double, 4, 3>> CalMat)
{
	...
	fout << "CameraNum" << MarkSiteSize;
	for (int i = 0; i < CalMat.size(); i++)
	{
		//sprintf_s(MatrixnumString, "Camera%d", i);
		fout << "Mat" << MyConverter::Matrix43dtoCvMat(CalMat[i]);
	}
	fout.release();
	return true;
}

CalibrateThread::CalibrateThread(WandCheckDatatypedef &cameraPara, string dir, int index)
{
	WandCheckData = cameraPara;
	FileDir = dir;
	FrameCount = 0;
...
void CalibrateThread::CalibrateImgProcess(std::vector<CirquePointtypedef> * CirquePointTree)
{
	...
	else
	{
		...
		{
			out(0, 2) = -1000;
			qDebug() << "最小偏差角" << MinCross << "直线拟合度不足" << endl;
		}
		else
		{
			double S[5];
